...
app.post(
  ...
  Transaction.insertFile_from_request_to_project_me
);
//
...
app.get(
  ...
  Transaction.getCountTransferlistaLLname
);
...
app.get(
  "/getData_Transfer_show_VIEW",
  VIEW_Fammaster.getData_Transfer_show_VIEW
);
...
app.use(
  "/downloads",
  express.static("/data/fixasset_service/API_SERVICE/Component/uploads/")
);
//getFAM_FILE_ATTACH
app.get("/downloads", (req, res) => {
  const fileName = req.query.filename;
  const filePath = path.join(
    "/data/fixasset_service/API_SERVICE/Component/uploads/",
    fileName
  );
